...
def main() -> int:
    ...
    for f in sorted(Path(args.results_dir).glob("*.json")):
        ...
        if "p80_error_ms" not in d:
            continue
        # Per-regime MAE is deliberately absent from the published summary:
        # differential resubmission inverts it to a single clip's exact answer,
        # and it labels that clip's regime. See score.py redacted().
        rows.append({
            "name": f.stem,
            "p80": d["p80_error_ms"],
            "n": d.get("n_scored"),
