...
def redacted(result: dict) -> dict:
    ...
    On a PUBLIC repository, Actions run logs and uploaded artifacts are
    readable by anyone, so anything a grading workflow emits is published.
    Published results are therefore the score and nothing else: no per-clip
    errors, no per-clip regime, no per-regime aggregates, and no mean.

    Candidates get the full per-regime and per-clip breakdown on the DEV set,
    where the answers are theirs already. The held-out number only has to
    rank them.
    """
...
def main():
    ...
    ap.add_argument("--public", action="store_true",
                    help="print only the score, no per-regime detail. Implied "
                         "by --summary-out.")
    args = ap.parse_args()
    ...
    if not public:
        # The grading workflow's stdout is a public Actions log, so these stay
        # out of it. See redacted().
